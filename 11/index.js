...
var processOptions = function(updated) {
	if (program.find) {
		console.log('Searching for commit \''+program.find+'\' in 11.x branch...')
		exec((!updated?"git checkout 11.x;":"")+"git log --pretty=oneline --since=\"2016-02-17\"", function (error, stdout, stderr) {
			var flines = processLines(stdout,function(t) { return t.indexOf(program.find) >= 0 });
			if (flines.length==0) { console.log('Commit \''+program.find+'\' not found in 11.x branch!'); }
			else {
				showLines('Found \''+program.find+'\' in 11.x branch',flines,true);
			}
		})
	} else {
		exec("git checkout 11.x",function (error, stdout, stderr) {
			if (error) {
				console.log(stderr);
				return;
			}
...
.option('-f, --find <find>', 'Find some commit in 11.x that matches pattern')
.option('-m, --missing','List missing commits in 11.x without jira or github issue reference')
.option('-c, --check <check>','Look for some missing commits in 11.x that matches pattern')
